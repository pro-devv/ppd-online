                                ...
                                <tbody>
                                    @forelse ($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ ucwords($item->no_hp) }}</td>
                                            <td>{{ $item->alamat }}</td>
                                            <td>
                                                <div class="d-flex">
                                                    {{-- <div class="p-1">
                                                        <a href="{{ route('data-ppd.edit',$item->id) }}" class="btn btn-warning"><i class="ti-pencil-alt"></i> Update data</a>
                                                    </div> --}}
                                                    <div class="p-1">
                                                        {{-- <form action="{{ route('user.destroy',$item->id) }}" method="POST"> --}}
                                                            {{-- @method('delete') --}}
                                                            {{-- @csrf --}}
                                                            <a href="{{ route('user.destroy',$item->id) }}" type="submit" class="btn btn-danger" onclick="return confirm('Hapus Data ?')"><i class="ti-trash"></i></a>
                                                        {{-- </form> --}}
                                                    </div>
                                                </div>
                                            </td>
